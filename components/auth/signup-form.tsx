  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      if (process.env.NODE_ENV === "production") {
        ...
        setLoading(false);
        setError(null);
        alert("Signup successful (dev mode - redirect disabled)");
      }
    }
  };
    ...
    <Card className="bg-stellar-navy/80 backdrop-blur-md border-stellar-glow/30 shadow-stellar-glow-lg">
      <CardHeader className="space-y-1 text-center">
        ...
        <CardDescription className="text-stellar-light/80">
          Create your account to begin your galactic journey
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSignup} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="fullName" className="text-stellar-light">
              Full Name
            </Label>
            <Input
              id="fullName"
